...
def build_WsHVR(input_shape, n_feature_maps, nb_classes, seed):
    ...
    conv1 = keras.layers.AveragePooling1D(pool_size=2, padding='same')(conv1)  # Using max pooling
    ...
    conv2 = keras.layers.Conv1D(n_feature_maps * 2, 3, 1, padding='same', kernel_regularizer=keras.regularizers.l2(0.01),
                                kernel_initializer=keras.initializers.he_normal(seed=seed))(conv1)
    ...
    # CRFRI = CRFRI_block(conv2)
    # CRFRI = keras.layers.Add()([conv2, CRFRI])
    ...
    # CRFRI = CRFRI_block(conv3)
    # CRFRI = keras.layers.Add()([conv3, CRFRI])
...
x, y = build_WsHVR(trainX_augmented.shape[1:], 128, nb_classes, seed)
...
# Save the model
model.save(f'C:/Users/cxq/Desktop/dataset_689/1m/modify model/average pooling/model_seed_{seed}.h5')

print(f"Seed: {seed}, Accuracy: {accuracy:.4f}")

# Load the model
model = keras.models.load_model(f'C:/Users/cxq/Desktop/dataset_689/1m/modify model/average pooling/model_seed_{seed}.h5')
